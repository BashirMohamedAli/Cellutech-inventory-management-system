...
export default function SidebarContent({ onClose, ...rest }) {
  ...
  return (
    <Box
      ...
      {...rest}
    >
      <Flex h='20' alignItems='center' mx='8' pt="1" justifyContent='space-between'>
        <Image src={logo} h="55" w="80"/>
        <CloseButton display={{ base: 'flex', md: 'none' }} onClick={onClose} />
      </Flex>
      {Links.map((link) => (
        <NavItem key={link.name} fontSize='xl' as='b' url={link.url} my="5" _hover={{ bg: "#273b9c", color: "white" }}> 
          {link.name}
        </NavItem>
      ))}
    </Box>
